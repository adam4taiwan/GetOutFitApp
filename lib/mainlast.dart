      ...
      home: Scaffold(
        ...
        bottomNavigationBar: BottomNavigationBar(
          items: const <BottomNavigationBarItem>[
            ...
            BottomNavigationBarItem( // 新增相機導航按鈕
...
class _CameraScreenState extends State<CameraScreen> {
    ...
    try {
      ...
      if (image != null) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('照片已拍攝並顯示！')),
        );
      } else {
        setState(() {
          _message = '未拍攝照片。';
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('錯誤: 無法開啟相機，請檢查權限。')),
      );
    }
  }
  ...
  void _handleUploadPhoto() async {
    ...
    try {
      ...
      if (image != null) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('照片已選擇並顯示！')),
        );
      } else {
        setState(() {
          _message = '未選擇照片。';
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('錯誤: 無法開啟相簿，請檢查權限。')),
      );
    }
  }
  ...
  void _handleSaveOutfit() {
    if (_selectedImage == null) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('請先拍攝或選擇一張照片！')),
      );
      return;
    }
    ...
    final newOutfit = {
      ...
      'imageUrl': 'https://placehold.co/300x200/A0BBE0/FFFFFF?text=AI推薦穿搭', // 模擬推薦圖片
    };
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      const SnackBar(content: Text('穿搭已保存到「我的穿搭」！')),
    );
  }
  ...
  Widget build(BuildContext context) {
    return Container(
      decoration: const BoxDecoration(
        gradient: LinearGradient(
          colors: [Color(0xFFF0F4C3), Color(0xFFC8E6C9)], // Light green/yellow gradient
          begin: Alignment.topLeft,
          end: Alignment.bottomRight,
        ),
      ),
                  ...
                  children: [
                        ...
                        child: _selectedImage != null
                            ...
                            : Center(
                                child: Column(
                                  mainAxisAlignment: MainAxisAlignment.center,
                                  children: [
                                    Icon(Icons.camera_alt, size: 80, color: Colors.grey[500]),
                                    const SizedBox(height: 16),
                                    Padding(
                    ...
                    Expanded(
                      ...
                      child: Column(
                        ...
                        children: [
                          _buildFunctionButton(context, '拍照', _handleCapturePhoto),
                          _buildFunctionButton(context, '上傳照片', _handleUploadPhoto),
                          _buildFunctionButton(context, '存入衣櫃', _handleSaveOutfit),
  ...
  Widget _buildFunctionButton(BuildContext context, String text, VoidCallback onPressed) {
    return Container(
      width: double.infinity,
